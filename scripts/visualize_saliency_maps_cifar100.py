...
class SaliencyVisualizerCIFAR100:
    def __init__(self, model, device='cpu'):
        ...
        # CIFAR-10 class names (what the model was trained on)
        self.cifar10_classes = [
            'airplane', 'automobile', 'bird', 'cat', 'deer',
            'dog', 'frog', 'horse', 'ship', 'truck'
        ]
        
        # CIFAR-100 class names (OOD data)
    ...
    def visualize_comparison(self, id_data, ood_data, num_samples=8, method='saliency', save_path=None):
        ...
        for i in range(min(num_samples, id_images.size(0), ood_images.size(0))):
            ...
            axes[0, i].set_title(f'ID: {self.cifar10_classes[id_labels[i]]}\nPred: {self.cifar10_classes[id_preds[i]]}', 
                               fontsize=10)
            axes[0, i].axis('off')
            ...
            axes[2, i].set_title(f'OOD: {self.cifar100_classes[ood_labels[i]]}\nPred: {self.cifar10_classes[ood_preds[i]]}', 
                               fontsize=9)
            axes[2, i].axis('off')
    ...
    def create_saliency_statistics(self, id_data, ood_data, method='saliency', save_path=None):
        ...
        axes[0, 0].bar(x + width/2, ood_values, width, label='OOD (CIFAR-100)', alpha=0.8)
        ...
        axes[0, 1].hist(ood_flat, bins=50, alpha=0.7, label='OOD (CIFAR-100)', density=True)
...
def load_datasets(batch_size=16):
    ...
    # Same transform for both datasets since they have similar characteristics
    ...
    # Create data loaders
    cifar10_loader = DataLoader(cifar10_test, batch_size=batch_size, shuffle=True)
    cifar100_loader = DataLoader(cifar100_test, batch_size=batch_size, shuffle=True)
    
    return cifar10_loader, cifar100_loader


def main():
    parser = argparse.ArgumentParser(description='Visualize Saliency Maps for CIFAR-10 vs CIFAR-100')
    ...
    parser.add_argument('--output_dir', type=str, default='saliency_cifar100', help='Output directory')
    ...
    cifar10_loader, cifar100_loader = load_datasets(args.batch_size)
    
    # Get sample batches
    id_batch = next(iter(cifar10_loader))
    ood_batch = next(iter(cifar100_loader))
    
    # Initialize visualizer
    visualizer = SaliencyVisualizerCIFAR100(model, device)
    
    # Create visualizations
    print(f"Creating {args.method} visualizations for CIFAR-10 vs CIFAR-100...")
    
    # 1. Side-by-side comparison
    comparison_path = os.path.join(args.output_dir, f'{args.method}_comparison_cifar100_{args.model_type}.png')
    ...
    stats_path = os.path.join(args.output_dir, f'{args.method}_statistics_cifar100_{args.model_type}.png')
    ...
    # Print statistics
    print(f"\n📊 {args.method.title()} Statistics (CIFAR-10 vs CIFAR-100):")
    print("ID (CIFAR-10):")
